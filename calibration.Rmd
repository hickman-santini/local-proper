...
```{r}
# Load Metaculus calibration data
# To regenerate this data, run: source("fetch_metaculus_data.R")
# Note: The fetch script uses question close time by default
# This can be configured by changing PREDICTION_TIME_POINT in fetch_metaculus_data.R

if (file.exists("metaculus_calibration_data.rds")) {
  df <- readRDS("metaculus_calibration_data.rds")
  cat("Loaded Metaculus data from RDS file\n")
} else if (file.exists("metaculus_calibration_data.csv")) {
  df <- read.csv("metaculus_calibration_data.csv")
  cat("Loaded Metaculus data from CSV file\n")
} else {
  # Fallback to original Manifold data if Metaculus data not available
  warning("Metaculus data not found. Using original Manifold data. Run fetch_metaculus_data.R to generate Metaculus data.")
  df <- data.frame(
...
ggplot(df, aes(x = prob_assigned, y = percent_resolved_pos)) +
  ...
  labs(x = "Prob Assigned", y = "Percent Resolved Pos", title = "Metaculus Community Calibration") +
  theme_clean()

ggsave("metaculus-calibration.png", width = 6, height = 6, dpi = 300)
```